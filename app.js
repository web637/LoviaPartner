'use strict';
let currentUser=null,currentPage='landing',musicPlaying=false,bookingStep=1,bookingData={},currentPriceFilter='all';
...
{id:'t001',name:'Ara Salsabila',nickname:'Ara',age:22,gender:'Perempuan',location:'Jakarta',bio:'Suka ngobrol, nonton film, dan kulineran. Aku orangnya humoris dan easygoing!',hobbies:'Film, Music, Kuliner',services:['Chatting','Calling','Video Call','Offline Date'],schedule:['Siang (12-17)','Sore (17-20)','Malam (20-24)'],rating:4.9,bookings:234,price:'26K',status:'online',avatar:'🌸',ig:'@ara.sal',tiktok:'@ara.salsabila',verified:true,username:'talent01',password:'talent123'},
{id:'t002',name:'Nara Putri',nickname:'Nara',age:20,gender:'Perempuan',location:'Bandung',bio:'Music lover dan gaming enthusiast. Yuk mabar bareng! Teman ngobrol yang seru.',hobbies:'Gaming, Music, Anime',services:['Chatting','Calling','Mabar','Video Call'],schedule:['Pagi (06-12)','Malam (20-24)'],rating:4.8,bookings:189,price:'26K',status:'online',avatar:'🎵',ig:'@naraputri',tiktok:'@nara.music',verified:true,username:'nara01',password:'nara123'},
...
{id:'t004',name:'Luna Safira',nickname:'Luna',age:21,gender:'Perempuan',location:'Yogyakarta',bio:'Introvert tapi asik diajak ngobrol. Suka sastra, kopi, dan hujan.',hobbies:'Membaca, Menulis, Kopi',services:['Chatting','Calling','Video Call'],schedule:['Sore (17-20)','Malam (20-24)'],rating:4.7,bookings:145,price:'26K',status:'offline',avatar:'🌙',ig:'@luna.safira',tiktok:'@luna_writes',verified:true,username:'luna01',password:'luna123'},
{id:'t005',name:'Kira Mahesa',nickname:'Kira',age:24,gender:'Laki-laki',location:'Bali',bio:'Pro gamer, bisa bantu carry rank kamu. Juga bisa jadi teman jalan yang asik!',hobbies:'Gaming, Surfing, Photography',services:['Mabar','Chatting','Offline Date'],schedule:['Pagi (06-12)','Malam (20-24)'],rating:4.8,bookings:278,price:'26K',status:'online',avatar:'🎮',ig:'@kira.mahesa',tiktok:'@kira_pro',verified:true,username:'kira01',password:'kira123'},
{id:'t006',name:'Reva Anindita',nickname:'Reva',age:22,gender:'Perempuan',location:'Jakarta',bio:'Aktris teater, punya banyak cerita seru. Yuk ngobrol dan temukan warna baru!',hobbies:'Teater, Seni, Kuliner',services:['Chatting','Calling','Video Call','Offline Date'],schedule:['Siang (12-17)','Sore (17-20)'],rating:4.9,bookings:201,price:'26K',status:'online',avatar:'🎭',ig:'@reva.anin',tiktok:'@reva_art',verified:true,username:'reva01',password:'reva123'},
...
{id:'t008',name:'Dani Pratama',nickname:'Dani',age:25,gender:'Laki-laki',location:'Semarang',bio:'Teman ngobrol yang hangat dan supportif. Pendengar terbaik buat kamu.',hobbies:'Olahraga, Musik, Traveling',services:['Chatting','Calling','Video Call','Offline Date'],schedule:['Pagi (06-12)','Sore (17-20)','Malam (20-24)'],rating:4.7,bookings:156,price:'26K',status:'online',avatar:'🌟',ig:'@dani.prat',tiktok:'@dani_vibe',verified:true,username:'dani01',password:'dani123'},
...
document.addEventListener('DOMContentLoaded',()=>{
  initLoading();initCursor();initNavbar();initTheme();renderHome();initCounters();initScrollReveal();initTyping();schedulePopup();
...
function initLoading(){setTimeout(()=>document.getElementById('loadingScreen').classList.add('hidden'),1900);}
...
function initCursor(){
  ...
  document.addEventListener('mouseover',e=>{if(e.target.matches('button,a,.talent-card,.service-card,.price-item-card,.package-card,.loc-card')){ring.style.width='50px';ring.style.height='50px';ring.style.borderColor='var(--pink-deep)';}});
  document.addEventListener('mouseout',e=>{if(e.target.matches('button,a,.talent-card,.service-card,.price-item-card,.package-card,.loc-card')){ring.style.width='32px';ring.style.height='32px';ring.style.borderColor='var(--pink)';}});
}

function initNavbar(){window.addEventListener('scroll',()=>document.getElementById('navbar').classList.toggle('scrolled',window.scrollY>20));}
...
document.getElementById('themeToggle').addEventListener('click',()=>{
  ...
  document.documentElement.setAttribute('data-theme',n);localStorage.setItem('lovia_theme',n);updateThemeIcon(n);
  toast('Mode '+(n==='dark'?'Gelap 🌙':'Terang ☀️')+' aktif','info');
});
function updateThemeIcon(t){document.getElementById('themeIcon').className=t==='dark'?'fas fa-sun':'fas fa-moon';}
...
function initTyping(){
  const phrases=['untuk Menemanimu 💫','untuk Ngobrol Seru 💬','untuk Offline Date 📍','untuk Mabar Bareng 🎮','Terbaik Untukmu ✨'];
...
function showPage(page){
  ...
  document.getElementById('mainFooter').style.display=dash?'none':'block';
  document.getElementById('navbar').style.display=dash?'none':'';
...
function renderHomeTalents(){document.getElementById('homeTalentGrid').innerHTML=getTalents().slice(0,4).map(talentCard).join('');}
function renderTestimonials(){
  document.getElementById('testimonialGrid').innerHTML=getTestimonials().map(t=>`
    <div class="testi-card reveal">
      <div class="testi-header">
        <div class="testi-avatar" style="background:linear-gradient(135deg,var(--pink-light),var(--purple-light))">${t.avatar}</div>
...
function talentCard(t){
  ...
  return `<div class="talent-card neon-hover" onclick="showTalentDetail('${t.id}')">
    <div class="talent-img">
      <span style="font-size:5rem">${t.avatar}</span>
      <div class="talent-status ${sc[t.status]||'status-offline'}">${si[t.status]||t.status}</div>
      ${t.verified?'<div style="position:absolute;top:.6rem;left:.6rem;background:rgba(59,130,246,.15);color:#3b82f6;border:1px solid rgba(59,130,246,.3);border-radius:50px;padding:.2rem .5rem;font-size:.65rem;font-weight:700">✓ Verified</div>':''}
    </div>
...
function renderSkeletonTalents(){
  document.getElementById('talentGrid').innerHTML=Array(8).fill(0).map(()=>`<div class="skeleton-card"><div class="skeleton skel-img"></div><div style="padding:1rem"><div class="skeleton skel-line"></div><div class="skeleton skel-line-sm"></div><div class="skeleton skel-line" style="width:75%"></div></div></div>`).join('');
  setTimeout(renderTalents,650);
}
function renderTalents(){document.getElementById('talentGrid').innerHTML=getTalents().map(talentCard).join('');setTimeout(initScrollReveal,60);}
function filterTalents(){
  const q=(document.getElementById('talentSearch').value||'').toLowerCase();
  const gender=document.getElementById('filterGender').value;
  const status=document.getElementById('filterStatus').value;
  const service=document.getElementById('filterService').value;
  const loc=document.getElementById('filterLocation').value;
  const res=getTalents().filter(t=>{
    if(q&&!t.name.toLowerCase().includes(q)&&!t.bio.toLowerCase().includes(q)&&!t.location.toLowerCase().includes(q))return false;
    if(gender&&t.gender!==gender)return false;
    if(status&&t.status!==status)return false;
    if(service&&!(t.services||[]).includes(service))return false;
    if(loc&&t.location!==loc)return false;
    return true;
  });
  document.getElementById('talentGrid').innerHTML=res.length?res.map(talentCard).join(''):'<div style="grid-column:1/-1;text-align:center;padding:4rem;color:var(--text-muted)"><div style="font-size:3rem;margin-bottom:1rem">🔍</div><h3>Tidak ditemukan</h3><p style="margin-top:.5rem">Coba ubah filter</p></div>';
}

function showTalentDetail(id){
  const t=getTalents().find(x=>x.id===id);if(!t)return;
  showPage('talent-detail');
  const revs=[{u:'Andin W.',s:5,tx:'Sangat menyenangkan dan profesional!',d:'3 hari lalu',av:'👩'},{u:'Bram S.',s:5,tx:'Ramah, asik, dan responsif banget!',d:'1 minggu lalu',av:'👦'},{u:'Cinta R.',s:4,tx:'Pengalaman sangat baik. Rekomen!',d:'2 minggu lalu',av:'👧'}];
  const sc={online:'status-online',offline:'status-offline',busy:'status-busy'};
  const si={online:'🟢 Online',busy:'🟠 Sibuk',offline:'⚫ Offline'};
  const vi={'Chatting':'💬','Calling':'📞','Video Call':'🎥','Offline Date':'📍','Mabar':'🎮'};
  document.getElementById('talentDetailContent').innerHTML=`
    <div class="talent-detail-layout">
      <div class="td-sidebar">
        <div class="td-main-img">${t.avatar}</div>
      ...
      <div class="td-main">
        <div class="td-info-card">
          ...
          <div class="gallery-grid">
            <div class="gallery-img">${t.avatar}</div>
            <div class="gallery-img" style="background:linear-gradient(135deg,var(--peach-light),var(--pink-light))">✨</div>
            <div class="gallery-img" style="background:linear-gradient(135deg,var(--purple-light),var(--cream))">🌟</div>
          </div>
        </div>
        <div class="td-info-card">
...
function renderPricelist(){
  ...
  document.getElementById('pricelistContainer').innerHTML=filtered.map(s=>{
    const items=pl[s.key]||[];
    return `<div class="price-section" data-cat="${s.key}">
...
function buildBookingModal(){
  ...
  document.getElementById('bookingContent').innerHTML=`
    ...
    <div class="booking-step active" id="bkStep1">
      ...
      ${talent?`<div style="display:flex;align-items:center;gap:1rem;padding:.85rem;background:var(--pink-light);border-radius:var(--radius-sm);margin-bottom:1.25rem"><span style="font-size:2rem">${talent.avatar}</span><div><strong>${talent.name}</strong><div style="font-size:.78rem;color:var(--text-muted)">${talent.location} · ⭐ ${talent.rating}</div></div></div>
      <div class="form-group"><label>Pilih Layanan</label><select id="bkService" style="width:100%;padding:.65rem 1rem;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--bg);color:var(--text)">${(talent.services||[]).map(s=>`<option>${s}</option>`).join('')}</select></div>`
      :`<div style="padding:.85rem;background:var(--purple-light);border-radius:var(--radius-sm);margin-bottom:1.25rem"><strong>📦 ${serviceName}</strong><div style="font-size:.82rem;color:var(--text-muted);margin-top:.25rem">Harga: Rp ${servicePrice}</div></div>
      <div class="form-group"><label>Pilih Talent (Opsional)</label><select id="bkTalent" style="width:100%;padding:.65rem 1rem;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--bg);color:var(--text)"><option value="">Pilihkan oleh admin</option>${getTalents().filter(t=>t.status==='online').map(t=>`<option value="${t.id}">${t.avatar} ${t.name} — ${t.location}</option>`).join('')}</select></div>`}
...
function bkNext(step){
  if(step===2&&!document.getElementById('bkDate').value){toast('Pilih tanggal dulu!','error');return;}
  document.getElementById('bkStep'+step).classList.remove('active');
  document.getElementById('bkStep'+(step+1)).classList.add('active');
...
function submitBookingFinal(){
  ...
  if(!name||!wa){toast('Lengkapi nama dan WhatsApp!','error');return;}
  const{talent,serviceName,servicePrice}=bookingData;
  const service=talent?(document.getElementById('bkService')||{}).value||'Chatting':serviceName;
  const talentName=talent?talent.name:(document.getElementById('bkTalent')||{}).value||'Ditentukan Admin';
  const date=(document.getElementById('bkDate')||{}).value||new Date().toISOString().split('T')[0];
  const orders=getOrders();
  orders.unshift({id:'ORD'+String(Date.now()).slice(-6),customer:name,wa,talent:talentName,service,date,location:(document.getElementById('bkLocation')||{}).value||'Online',note:(document.getElementById('bkNote')||{}).value||'',status:'Menunggu',total:servicePrice||(talent?talent.price:'0'),createdAt:new Date().toISOString()});
  setOrders(orders);closeModal('bookingModal');toast('Booking berhasil dikirim! 🎉','success');
  setTimeout(()=>showNotifModal(`Booking <strong>${service}</strong> dengan <strong>${talentName}</strong> berhasil!<br><br>Konfirmasi dikirim ke WhatsApp <strong>${wa}</strong> dalam 5–15 menit.`,talent?talent.avatar:'💝'),400);
}

function regNext(step){
  if(step===1){
    if(!document.getElementById('reg_nama').value.trim()||!document.getElementById('reg_umur').value||!document.getElementById('reg_gender').value||!document.getElementById('reg_kota').value.trim()||!document.getElementById('reg_wa').value.trim()||!document.getElementById('reg_email').value.trim()){toast('Lengkapi semua field *!','error');return;}
    if(+document.getElementById('reg_umur').value<18){toast('Minimal usia 18 tahun!','error');return;}
  }else if(step===2){if(document.getElementById('reg_bio').value.trim().length<20){toast('Bio minimal 20 karakter!','error');return;}}
  goRegStep(step+1);
}
...
function submitRegister(){
  ...
  const emojis=['🌸','🌺','🌙','⭐','✨','🎵','💫','🦋'];
  const newT={id:newId,name:document.getElementById('reg_nama').value.trim(),nickname:document.getElementById('reg_panggilan').value.trim(),age:+document.getElementById('reg_umur').value,gender:document.getElementById('reg_gender').value,location:document.getElementById('reg_kota').value.trim(),bio:document.getElementById('reg_bio').value.trim(),hobbies:'Belum diisi',services,schedule,rating:0,bookings:0,price:'26K',status:'offline',avatar:emojis[Math.floor(Math.random()*emojis.length)],verified:false,pendingApproval:true,ig:document.getElementById('reg_ig').value||'',tiktok:document.getElementById('reg_tiktok').value||'',username:'talent_'+newId,password:pass,email:document.getElementById('reg_email').value.trim(),wa:document.getElementById('reg_wa').value.trim()};
  talents.push(newT);setTalents(talents);
  toast('Pendaftaran berhasil! 🎉','success');
  showNotifModal(`Pendaftaran berhasil!<br><br>Admin akan menghubungi via WhatsApp dalam 1×24 jam.<br><br><strong>Username:</strong> ${newT.username}<br><strong>Password:</strong> ${pass}<br><small style="color:var(--text-muted)">Simpan kredensial ini dengan aman</small>`,'🌟');
...
function handleLogin(){
  ...
  if(!user||!pass){toast('Isi username dan password!','error');return;}
  ...
  if(t){
    ...
    currentUser={role:'talent',name:t.name,talentId:t.id,username:t.username};lsSet('lovia_session',currentUser);closeModal('loginModal');
    toast(`Selamat datang, ${t.nickname||t.name}! ✨`,'success');setTimeout(()=>showPage('talent-dash'),500);return;
  }
...
function renderAdminOverview(el){
  ...
  el.innerHTML=`
    ...
    <div class="dash-grid-4">
      ...
      <div class="dash-stat-card" style="border-top:3px solid var(--purple-deep)"><div class="dsc-icon">📦</div><div class="dsc-val">${orders.length}</div><div class="dsc-label">Total Pesanan</div></div>
      <div class="dash-stat-card" style="border-top:3px solid #f59e0b"><div class="dsc-icon">⏳</div><div class="dsc-val">${pending.length}</div><div class="dsc-label">Menunggu Seleksi</div></div>
    </div>
    ${pending.length?`<div class="dash-section" style="border-left:4px solid var(--pink-deep)"><h3>🔔 Pendaftaran Menunggu <span style="background:var(--pink-deep);color:#fff;padding:.15rem .5rem;border-radius:50px;font-size:.7rem;margin-left:.5rem">${pending.length}</span></h3><div class="table-scroll"><table class="admin-table"><thead><tr><th>Nama</th><th>Kota</th><th>Layanan</th><th>WhatsApp</th><th>Aksi</th></tr></thead><tbody>${pending.map(t=>`<tr><td><strong>${t.name}</strong><br><small style="color:var(--text-muted)">${t.age}thn · ${t.gender}</small></td><td>${t.location}</td><td style="font-size:.78rem">${(t.services||[]).join(', ')}</td><td style="font-size:.78rem">${t.wa||'—'}</td><td style="display:flex;gap:.35rem"><button class="btn-sm" onclick="approveTalent('${t.id}')" style="border-color:#48bb78;color:#48bb78">✓</button><button class="btn-sm" onclick="rejectTalent('${t.id}')" style="border-color:#ef4444;color:#ef4444">✕</button></td></tr>`).join('')}</tbody></table></div></div>`:''}
    <div class="dash-section"><h3>🛒 Pesanan Terbaru</h3><div class="table-scroll">${ordersTable(orders.slice(0,5))}</div></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
      <div class="dash-section"><h3>📊 Statistik Talent</h3><div style="font-size:.85rem;display:flex;flex-direction:column;gap:.5rem"><div style="display:flex;justify-content:space-between"><span>Verified</span><strong>${talents.filter(t=>t.verified).length}</strong></div><div style="display:flex;justify-content:space-between"><span>Perempuan</span><strong>${talents.filter(t=>t.gender==='Perempuan').length}</strong></div><div style="display:flex;justify-content:space-between"><span>Laki-laki</span><strong>${talents.filter(t=>t.gender==='Laki-laki').length}</strong></div><div style="display:flex;justify-content:space-between"><span>Rating ≥4.8</span><strong>${talents.filter(t=>t.rating>=4.8).length}</strong></div></div></div>
...
function filterAdminOrders(status){const orders=status?getOrders().filter(o=>o.status===status):getOrders();document.getElementById('ordersTableWrap').innerHTML=ordersTable(orders,true);}
...
function renderAdminPricelist(el){
  ...
  el.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;flex-wrap:wrap;gap:.75rem"><h2 style="font-family:var(--font-display)">Kelola Pricelist 💰</h2><span style="font-size:.8rem;color:var(--text-muted)">Edit langsung → tersimpan otomatis</span></div>
    ...
    <div class="dash-section"><h3>💡 Tips</h3><p style="font-size:.85rem;color:var(--text-muted)">Paket Relationship & PDKT dikelola dari DEFAULT_PRICELIST di app.js. Gunakan Reset Data di Pengaturan untuk kembali ke harga awal.</p></div>`;
}
...
function renderAdminSettings(el){
  el.innerHTML=`<h2 style="font-family:var(--font-display);margin-bottom:1.5rem">Pengaturan ⚙️</h2>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem">
      ...
      <div class="dash-section"><h3>📊 Platform</h3><div style="font-size:.88rem;display:flex;flex-direction:column;gap:.5rem"><div>Versi: <strong>Lovia Partner v2.0</strong></div><div>Storage: <strong>LocalStorage</strong></div><div>Deploy: <strong>GitHub Pages Ready</strong></div></div></div>
...
function showTalentTab(tab){
  ...
  document.getElementById('talent-tab-'+tab).classList.add('active');
  ...
  el.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem">
      <div class="dash-section" style="background:linear-gradient(135deg,var(--pink-light),var(--purple-light))">
        ...
        <div style="font-family:var(--font-display);font-size:1.1rem">${t.name}</div>
        <div style="font-size:.8rem;color:var(--text-muted);margin:.3rem 0">⭐ ${t.rating} · 📍 ${t.location}</div>
        <div style="display:flex;flex-wrap:wrap;gap:.3rem;margin-top:.6rem">${(t.services||[]).map(s=>`<span style="padding:.2rem .6rem;background:rgba(255,255,255,.5);border-radius:50px;font-size:.7rem">${s}</span>`).join('')}</div>
      </div>
...
function toggleTalentStatus(id){const t=getTalents();const i=t.findIndex(x=>x.id===id);if(i>=0){t[i].status=t[i].status==='online'?'offline':'online';setTalents(t);}toast(`Status: ${t[i].status}`,'info');renderTalentDash();}
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem">
      <div class="dash-section">
        <div style="text-align:center;padding:1.5rem;background:linear-gradient(135deg,var(--pink-light),var(--purple-light));border-radius:var(--radius);margin-bottom:1.25rem">
          <div style="font-size:4rem;margin-bottom:.5rem">${t.avatar}</div>
...
function saveTalentProfile(id){const t=getTalents();const i=t.findIndex(x=>x.id===id);if(i>=0){t[i].bio=document.getElementById('editBio').value;t[i].hobbies=document.getElementById('editHobi').value;t[i].ig=document.getElementById('editIg').value;t[i].tiktok=document.getElementById('editTiktok').value;setTalents(t);}toast('Profil tersimpan! ✓','success');}
...
function toggleMusic(){
  ...
  document.getElementById('musicIcon').className=musicPlaying?'fas fa-pause':'fas fa-music';
  document.getElementById('floatMusic').classList.toggle('playing',musicPlaying);
...
function showPopup(icon,title,body){
  ...
  document.getElementById('pnIcon').textContent=icon+' ';document.getElementById('pnTitle').textContent=title;document.getElementById('pnBody').textContent=body;
